...
async function loadDefaultConfig(dir, configName = 'promptfooconfig') {
    ...
    if (exports.configCache.has(cacheKey)) {
        return exports.configCache.get(cacheKey);
    }
    let defaultConfig = {};
    ...
    for (const ext of extensions) {
        const configPath = path_1.default.join(dir, `${configName}.${ext}`);
        const maybeConfig = await (0, load_1.maybeReadConfig)(configPath);
        if (maybeConfig) {
    ...
    exports.configCache.set(cacheKey, result);
    return result;
}
function clearConfigCache() {
    exports.configCache.clear();
}
