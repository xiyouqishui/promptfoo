...
describe('RedteamIterativeProvider', () => {
    describe('renderSystemPrompts', () => {
        it('should render system prompts correctly with default settings', () => {
            ...
            expect(result.onTopicSystemPrompt).toBe(prompts_1.ON_TOPIC_SYSTEM_PROMPT);
    ...
    describe('getNewPrompt', () => {
        ...
        it('should throw an error for invalid API response', async () => {
            ...
            await expect((0, iterativeTree_1.getNewPrompt)(mockRedteamProvider, redteamHistory)).rejects.toThrow();
        });
    ...
    describe('checkIfOnTopic', () => {
        ...
        it('should return true for on-topic prompt', async () => {
            ...
            const result = await (0, iterativeTree_1.checkIfOnTopic)(mockRedteamProvider, 'On-topic system prompt', 'Target prompt');
            expect(result).toMatchObject({ isOnTopic: true });
            expect(mockRedteamProvider.callApi).toHaveBeenCalledTimes(1);
            expect(mockRedteamProvider.callApi).toHaveBeenCalledWith('[{"role":"system","content":"On-topic system prompt"},{"role":"user","content":"Target prompt"}]', expect.objectContaining({
                prompt: expect.objectContaining({
                    label: 'on-topic',
        ...
        it('should return false for off-topic prompt', async () => {
            ...
            const result = await (0, iterativeTree_1.checkIfOnTopic)(mockRedteamProvider, 'On-topic system prompt', 'Off-topic prompt');
            expect(result).toMatchObject({ isOnTopic: false });
        });
        it('should throw an error for invalid API response', async () => {
            mockRedteamProvider.callApi.mockResolvedValue({ output: 'invalid json' });
            await expect((0, iterativeTree_1.checkIfOnTopic)(mockRedteamProvider, 'On-topic system prompt', 'Target prompt')).rejects.toThrow(); // eslint-disable-line jest/require-to-throw-message
        });
        it('should throw an error for unexpected API response format', async () => {
            mockRedteamProvider.callApi.mockResolvedValue({
                output: JSON.stringify({ unexpectedKey: true }),
            });
            await expect((0, iterativeTree_1.checkIfOnTopic)(mockRedteamProvider, 'On-topic system prompt', 'Target prompt')).rejects.toThrow('Invariant failed: Expected onTopic to be a boolean');
        });
    });
    describe('updateRedteamHistory', () => {
        it('should create a new history entry with correct content', () => {
            ...
            const result = (0, iterativeTree_1.updateRedteamHistory)(targetResponse, goal, score, isOnTopic, containsPenalizedPhrase, excludeTargetOutput);
        ...
        it('should handle off-topic and penalized responses', () => {
            ...
            expect(result.content).toContain(`OBJECTIVE: ${goal}`);
            expect(result.content).toContain(`SCORE: ${score} (your prompt was off-topic) (contains penalized phrase)`);
        });
...
describe('Tree Structure', () => {
    ...
    describe('selectNodes', () => {
        ...
        it('should mark selected nodes in treeOutputs', async () => {
            ...
            const selectedNodes = await (0, iterativeTree_1.selectNodes)(nodes, mockRedteamProvider, 'test prompt', 'test goal');
