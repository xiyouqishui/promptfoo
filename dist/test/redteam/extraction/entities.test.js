...
describe('Entities Extractor', () => {
    ...
    it('should ignore Nunjucks template variables in double curly braces', async () => {
        ...
        const result = await (0, entities_1.extractEntities)(provider, [
    ...
    it('should handle errors in local extraction', async () => {
        ...
        const result = await (0, entities_1.extractEntities)(provider, ['prompt']);
        expect(result).toEqual([]);
        expect(logger_1.default.warn).toHaveBeenCalledWith(expect.stringContaining('Error using local extraction'));
    });
});
