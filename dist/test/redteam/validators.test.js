...
describe('redteamConfigSchema', () => {
    ...
    it('should allow all valid plugin and strategy names', () => {
        const strategiesExceptDefault = constants_1.ALL_STRATEGIES.filter((id) => id !== 'default' && id !== 'basic');
        const input = {
        ...
        expect(result).toEqual({
            ...
            data: expect.objectContaining({
                numTests: undefined,
                plugins: expect.arrayContaining(constants_1.ALL_PLUGINS.filter((id) => !constants_1.COLLECTIONS.includes(id)).map((id) => ({
                    id,
                }))),
                strategies: expect.arrayContaining(strategiesExceptDefault.map((id) => ({ id }))),
            }),
        });
        expect(result.data?.plugins).toHaveLength(constants_1.ALL_PLUGINS.filter((id) => !constants_1.COLLECTIONS.includes(id)).length);
    ...
    describe('aliases', () => {
        ...
        it('should not duplicate plugins when using multiple aliased names', () => {
            ...
            for (const expected of expectedPluginObjects) {
                const found = actualPlugins.find((p) => p.id === expected.id && p.numTests === expected.numTests);
                if (!found) {
                    throw new Error(`Expected to find plugin ${expected.id} with numTests=${expected.numTests}, but it was missing`);
                }
            }
...
describe('RedteamConfigSchema transform', () => {
    ...
    it('should expand collection plugins correctly', () => {
        ...
        expect(result.plugins
            ...
            .every((p) => p.id.startsWith('harmful:'))).toBe(true);
