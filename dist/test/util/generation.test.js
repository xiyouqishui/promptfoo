...
const generation_1 = require("../../src/util/generation");
describe('retryWithDeduplication', () => {
    it('should collect unique items until target count is reached', async () => {
        ...
        const result = await (0, generation_1.retryWithDeduplication)(operation, 5);
    ...
    it('should stop after max consecutive retries', async () => {
        const operation = jest
            ...
            .mockResolvedValueOnce([1, 2])
            .mockResolvedValueOnce([1, 2]);
        const result = await (0, generation_1.retryWithDeduplication)(operation, 5, 2);
    ...
    it('should use custom deduplication function', async () => {
        ...
        const customDedupFn = (items) => Array.from(new Set(items.map((item) => item.id))).map((id) => ({ id }));
        const result = await (0, generation_1.retryWithDeduplication)(operation, 3, 2, customDedupFn);
    ...
    it('should handle empty results from operation', async () => {
        ...
        const result = await (0, generation_1.retryWithDeduplication)(operation, 3);
    ...
    it('should return all unique items even if target count is not reached', async () => {
        ...
        const result = await (0, generation_1.retryWithDeduplication)(operation, 10, 2);
...
describe('sampleArray', () => {
    it('should return n random items when n is less than array length', () => {
        ...
        const result = (0, generation_1.sampleArray)(array, 3);
    ...
    it('should return all items when n is equal to array length', () => {
        const array = [1, 2, 3, 4, 5];
        const result = (0, generation_1.sampleArray)(array, 5);
    ...
    it('should return an empty array when input array is empty', () => {
        const result = (0, generation_1.sampleArray)([], 3);
        expect(result).toEqual([]);
    });
    it('should return a new array, not modifying the original', () => {
        const array = [1, 2, 3, 4, 5];
        const originalArray = [...array];
        (0, generation_1.sampleArray)(array, 3);
