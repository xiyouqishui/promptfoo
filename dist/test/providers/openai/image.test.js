...
describe('OpenAiImageProvider', () => {
    ...
    describe('Basic functionality', () => {
        it('should generate an image successfully', async () => {
            const provider = new image_1.OpenAiImageProvider('dall-e-3', {
                config: { apiKey: 'test-key' },
            });
            const result = await provider.callApi('Generate a cat');
            expect(cache_1.fetchWithCache).toHaveBeenCalledWith(expect.stringContaining('/images/generations'), expect.objectContaining({
                method: 'POST',
        ...
        it('should throw an error if API key is not set', async () => {
            ...
            try {
                ...
                await expect(provider.callApi('Generate a cat')).rejects.toThrow('OpenAI API key is not set. Set the OPENAI_API_KEY environment variable or add `apiKey` to the provider config.');
            }
            finally {
    ...
    describe('Response format handling', () => {
        it('should handle base64 response format', async () => {
            ...
            jest.mocked(cache_1.fetchWithCache).mockResolvedValue(mockBase64Response);
            const result = await provider.callApi('test prompt');
            expect(result).toEqual({
    ...
    describe('Parameter validation', () => {
        ...
        it('should validate size for DALL-E 2', async () => {
            const provider = new image_1.OpenAiImageProvider('dall-e-2', {
    ...
    describe('Operation handling', () => {
        it('should reject non-generation operations', async () => {
            ...
            const result = await provider.callApi('test prompt');
            expect(result).toHaveProperty('error');
            expect(result.error).toContain("Only 'generation' operations are currently supported. 'variation' operations are not implemented.");
        });
    });
    describe('Configuration options', () => {
        ...
        it('should use custom API URL when provided', async () => {
            ...
            await provider.callApi('test prompt');
            expect(cache_1.fetchWithCache).toHaveBeenCalledWith(`${customApiUrl}/images/generations`, expect.any(Object), expect.any(Number));
        });
    });
});
