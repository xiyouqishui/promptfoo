...
describe('HttpProvider', () => {
    ...
    describe('createtransformResponse', () => {
        it('should handle function parser', async () => {
            const functionParser = (data) => data.result;
        ...
        it('should handle file:// parser with JavaScript file', async () => {
            ...
            const parser = await (0, http_1.createTransformResponse)('file://custom-parser.js');
        ...
        it('should throw error for unsupported parser type', async () => {
            await expect((0, http_1.createTransformResponse)(123)).rejects.toThrow("Unsupported response transform type: number. Expected a function, a string starting with 'file://' pointing to a JavaScript file, or a string containing a JavaScript expression.");
        });
        ...
        it('should handle file:// parser with specific function name', async () => {
            ...
            jest.mocked(esm_1.importModule).mockResolvedValueOnce(mockParser);
            const parser = await (0, http_1.createTransformResponse)('file://custom-parser.js:parseResponse');
        ...
        it('should throw error for malformed file:// parser', async () => {
            ...
            await expect((0, http_1.createTransformResponse)('file://invalid-parser.js')).rejects.toThrow(/Response transform malformed/);
        });
        it('should return default parser when no parser is provided', async () => {
            const parser = await (0, http_1.createTransformResponse)(undefined);
        ...
        it('should handle response transform file with default export', async () => {
            ...
            jest.mocked(esm_1.importModule).mockResolvedValueOnce(mockParser);
            const parser = await (0, http_1.createTransformResponse)('file://default-parser.js');
            const result = parser({ defaultField: 'parsed' }, '');
            expect(result).toBe('parsed');
            expect(esm_1.importModule).toHaveBeenCalledWith(path_1.default.resolve('/mock/base/path', 'default-parser.js'), undefined);
        });
    });
    ...
    it('should handle response transform returning an object', async () => {
        const provider = new http_1.HttpProvider(mockUrl, {
            config: {
                body: { key: 'value' },
                transformResponse: (json, text) => ({ custom: json.result }),
            },
        });
        const mockResponse = {
            data: JSON.stringify({ result: 'success' }),
        ...
        it('should prefer transformResponse over responseParser when both are set', async () => {
            const provider = new http_1.HttpProvider(mockUrl, {
                config: {
                    ...
                    headers: { 'Content-Type': 'application/json' },
                    body: { key: '{{ prompt }}' },
                    responseParser: (data) => ({ chat_history: 'from responseParser' }),
                    transformResponse: (data) => ({ chat_history: 'from transformResponse' }),
                },
            });
            const mockResponse = {
                data: JSON.stringify({ result: 'success' }),
...
describe('createTransformRequest', () => {
    it('should return identity function when no transform specified', async () => {
        const transform = await (0, http_1.createTransformRequest)(undefined);
        const result = await transform('test prompt');
        expect(result).toBe('test prompt');
    });
    it('should handle string templates', async () => {
        const transform = await (0, http_1.createTransformRequest)('return {"text": prompt}');
    ...
    it('should handle errors in function-based transform', async () => {
        ...
        const transform = await (0, http_1.createTransformRequest)(errorFn);
        await expect(async () => {
    ...
    it('should handle errors in file-based transform', async () => {
        ...
        jest.mocked(esm_1.importModule).mockResolvedValueOnce(mockErrorFn);
        const transform = await (0, http_1.createTransformRequest)('file://error-transform.js');
        await expect(async () => {
            await transform('test');
    ...
    it('should handle errors in string template transform', async () => {
        const transform = await (0, http_1.createTransformRequest)('return badVariable.nonexistent');
        await expect(async () => {
            await transform('test');
    ...
    it('should throw error for unsupported transform type', async () => {
        await expect((0, http_1.createTransformRequest)(123)).rejects.toThrow('Unsupported request transform type: number');
    });
    it('should include filename in error for file-based transform errors', async () => {
        ...
        jest.mocked(esm_1.importModule).mockResolvedValueOnce(mockErrorFn);
        const transform = await (0, http_1.createTransformRequest)('file://specific-file.js');
        await expect(async () => {
            await transform('test');
    ...
    it('should handle errors in string template rendering', async () => {
        const transform = await (0, http_1.createTransformRequest)('{{ nonexistent | invalid }}');
        await expect(async () => {
            await transform('test');
        }).rejects.toThrow('Error in request transform string template: (unknown path)\n  Error: filter not found: invalid');
    });
});
...
describe('transform request error handling', () => {
    ...
    it('should throw error for malformed file-based request transform', async () => {
        jest.mocked(esm_1.importModule).mockRejectedValueOnce(new Error('Module not found'));
        await expect((0, http_1.createTransformRequest)('file://invalid-transform.js')).rejects.toThrow('Module not found');
    });
});
