...
describe('PromptfooHarmfulCompletionProvider', () => {
    ...
    it('should handle successful API call', async () => {
        ...
        jest.mocked(fetch_1.fetchWithRetries).mockResolvedValue(mockResponse);
...
describe('PromptfooSimulatedUserProvider', () => {
    ...
    const provider = new promptfoo_1.PromptfooSimulatedUserProvider(options);
    it('should return correct id', () => {
        expect(provider.id()).toBe('test-agent');
    });
    it('should return default id if not provided', () => {
        const defaultProvider = new promptfoo_1.PromptfooSimulatedUserProvider();
    ...
    it('should handle API call exception', async () => {
        jest.mocked(fetch_1.fetchWithRetries).mockRejectedValue(new Error('Network Error'));
        const result = await provider.callApi(JSON.stringify([{ role: 'user', content: 'hello' }]));
